from model_xf.model.model_dl_base import PredictionLayer,CrossNet,DNN
from model_xf.model.data_process.dl_embedding import build_input_features,combined_dnn_input_dense_embed,\
    embedding_input_from_feature_columns,get_dense_input,get_linear_logit
from keras.models import Model
from model_xf.common.uitls import auc,concat_fun
...
class Deep_Cross:
    def __init__(self,model_config):
        ...
        self.use_cross = model_config['use_cross']
        ...
        #fm部分
        cross_input = concat_fun(self.sparse_embedding_list+self.dense_embedding_list, axis=1)
        cross_logit = CrossNet()(cross_input)
        ...
        if len(self.dnn_hidden_units) == 0 and self.use_cross == False:  # only linear
            final_logit = linear_logit
        elif len(self.dnn_hidden_units) == 0 and self.use_cross == True:  # linear + cross
            final_logit = keras.layers.add([linear_logit, cross_logit])
        elif len(self.dnn_hidden_units) > 0 and self.use_cross == False:  # linear + Deep
            final_logit = keras.layers.add([linear_logit, dnn_logit])
        elif len(self.dnn_hidden_units) > 0 and self.use_cross == True:  # linear + cross + Deep
            final_logit = keras.layers.add([linear_logit, cross_logit, dnn_logit])
        else:
    ...
    def model_(self):
        print('=' * 10, '> construct DeepCross')
        output = PredictionLayer(self.task,name=self.prefix)(self.final_logit)
        self.output = output
    ...
    def train(self,train_model_input,train_target):
        ...
        if self.use_global_epochs:
            ...
            for item in self.model.layers:
                print(item.name)
                print(isinstance(item, engine.input_layer.InputLayer))
                if isinstance(item, engine.input_layer.InputLayer):
                    print("input_layer")
                else:
